...
#define ZRMOT_DUR                                                              \
  0x22 // Duration counter threshold for zero motion interrupt generation, 16 Hz
       // rate, LSB = 64 ms
...
// mbed uses the eight-bit device address, so shift seven-bit addresses left by
// one!
#define ADO 0
#if ADO
...
extern uint8_t Mscale; // MFS_14BITS or MFS_16BITS, 14-bit or 16-bit
                       // magnetometer resolution
extern uint8_t
    Mmode; // Either 8 Hz 0x02) or 100 Hz (0x06) magnetometer data ODR
...
// Pin definitions
extern int
    intPin; // These can be changed, 2 and 3 are the Arduinos ext int pins

extern int16_t
    accelCount[3]; // Stores the 16-bit signed accelerometer sensor output
extern int16_t gyroCount[3]; // Stores the 16-bit signed gyro sensor output
extern int16_t
    magCount[3]; // Stores the 16-bit signed magnetometer sensor output
extern float magCalibration[3],
    magbias[3]; // Factory mag calibration and mag bias
extern float gyroBias[3],
    accelBias[3]; // Bias corrections for gyro and accelerometer
extern float ax, ay, az, gx, gy, gz, mx, my,
...
extern float PI;
extern float GyroMeasError; // gyroscope measurement error in rads/s (start at
                            // 60 deg/s), then reduce after ~10 s to 3
extern float beta;          // compute beta
extern float GyroMeasDrift; // gyroscope measurement drift in rad/s/s (start at
                            // 0.0 deg/s/s)
extern float zeta; // compute zeta, the other free parameter in the Madgwick
                   // scheme usually set to a small or zero value
#define Kp                                                                     \
  2.0f * 5.0f // these are the free parameters in the Mahony filter and fusion
...
extern int lastUpdate, firstUpdate,
    Now; // used to calculate integration interval // used to calculate
...
public:
  ...
  void readAccelData(int16_t *destination) {
    ...
    destination[0] = (int16_t)(((int16_t)rawData[0] << 8) |
  ...
  void readMagData(int16_t *destination) {
    ...
    if (readByte(AK8963_ADDRESS, AK8963_ST1) &
        0x01) { // wait for magnetometer data ready bit to be set
      readBytes(AK8963_ADDRESS, AK8963_XOUT_L, 7,
                &rawData[0]); // Read the six raw data and ST2 registers
      ...
      if (!(c & 0x08)) { // Check if magnetic sensor overflow set, if not then
        ...
        destination[0] = (int16_t)(((int16_t)rawData[1] << 8) |
                                   rawData[0]); // Turn the MSB and LSB into a
                                                // signed 16-bit value
  ...
  int16_t readTempData() {
    ...
    readBytes(MPU9250_ADDRESS, TEMP_OUT_H, 2,
              &rawData[0]); // Read the two raw data registers sequentially into
                            // data array
    return (int16_t)(((int16_t)rawData[0]) << 8 |
  ...
  void initAK8963(float *destination) {
    ...
    destination[0] = (float)(rawData[0] - 128) / 256.0f +
                     1.0f; // Return x-axis sensitivity adjustment values, etc.
  ...
  void initMPU9250() {
    ...
    wait_us(100000); // Delay 100 ms for PLL to get established on x-axis gyro;
  ...
  void calibrateMPU9250(float *dest1, float *dest2) {
    ...
    writeByte(MPU9250_ADDRESS, FIFO_EN,
              0x78); // Enable gyro and accelerometer sensors for FIFO (max size
                     // 512 bytes in MPU-9250)
    ...
    */
    dest1[0] = (float)gyro_bias[0] /
               (float)gyrosensitivity; // construct gyro bias in deg/s for later
                                       // manual subtraction
                          ...
                          // values, +/- 14 or less deviation is a pass
  {
    ...
    writeByte(MPU9250_ADDRESS, ACCEL_CONFIG,
              0xE0); // Enable self test on all three axes and set accelerometer
                     // range to +/- 2 g
    writeByte(MPU9250_ADDRESS, GYRO_CONFIG,
              0xE0); // Enable self test on all three axes and set gyro range to
                     // +/- 250 degrees/s
    ...
    for (int i = 0; i < 3; i++) {
      ...
      destination[i + 3] =
          100.0 * ((float)(gSTAvg[i] - gAvg[i])) / factoryTrim[i + 3] -
          100.; // Report percent differences
    }
  }
